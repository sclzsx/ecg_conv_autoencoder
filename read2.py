...
def preprocess_dataset2(data_path):
    # read pkl
    with open(os.path.join(data_path, 'mit-bih-database.pkl'), 'rb') as fin:
        res = dill.load(fin)
    ...
    print(all_data.shape, all_label.shape, all_peak.shape)
    print(all_data[0].shape, len(all_label[0]), len(all_peak[0]))
    # h,w = all_data.shape
    # for i in range(h):
